...
const mapCurrentEvents = () => {
  ...
  let constructedEvents = currentEvents.news.map(event => {
...
function Event(obj){
  ...
  this.img_url = obj.image || 'no image available';
...
const createServer = () => {
  ...
  function getCurrentEvents(request, response){
    mapCurrentEvents();
    console.log('MCE: ', mapCurrentEvents()[0].author);
    response.send('/events');
  }


  // Solution above, Server start below
...
const countNumberOfChildren = (arr) => {
  ...
  const red = (acc, curVal) => {
    if (curVal.children) {
      let counted = curVal.children;
      totals.push(counted.length);
    }
  };

  arr.reduce(red, 1[0]);

  let solution = totals.reduce((a, b) => a + b, 0);
  return solution;
};
...
const countPrimeNumbers = (arr) => {
  ...
  let reduc = arr.reduce((acc, cur, idx, src) => {
    if (isPrime(cur)) {
      return count += 1
    }
  }, count);
