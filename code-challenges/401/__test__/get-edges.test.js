...
const hasDirectFlights = require('../getEdges/get-edges.js').hasDirectFlights;
...
describe('Get Edges from Itinerary', () => {
  ...
  it('should return true with the price for flights with destinations that are a direct connection to the origin', () => {
    ...
    let result = hasDirectFlights(graph, arr);
  ...
  it('should return true with the summed price for more than 2 flights that are directly connected to one another', () => {
    ...
    let arr = [la.value, sea.value, chi.value, nyc.value];
    let result = hasDirectFlights(graph, arr);

    expect(result).toEqual([true, 325]);
  });
});
