...
const createServer = () => {
  ...
  function send405(req, res) {
    res.status(405).send('oops! can\'t do that.');
  }
...
const toTitleCase = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    let str = arr[i];
    console.log(str);
    let capped = str.replace( (str.charAt(0)), (str.charAt(0).toUpperCase()) );
...
const isSecure = (url) => {
  // Solution code here...
  let re = /^https:\/\/(www\.)?[a-zA-Z0-9]{1,100}\.[a-zA-Z0-9]{1,10}$/;
